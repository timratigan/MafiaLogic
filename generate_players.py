...
def generate_role_counts(GOD_RANDOMNESS=0.4,MAX_ROLE_COUNT=3,known_counts={},print_flag=False):
    ...
    for role_index in range(num_roles):
        role_stats = role_stats_list[role_index]
        role_counts.append(np.round(random.gauss(role_stats["Average_Number"],role_stats["Average_Number"]*GOD_RANDOMNESS)))
        # Enforce minimum/maximum roles:
        role_counts[role_index] = min(max(role_counts[role_index],role_stats["Min_Number"]),MAX_ROLE_COUNT)

    # Enforce the known role counts:
    for (role_name, role_count) in known_counts.items():
        role_counts[role_names.index(role_name)]=role_count
        role_info[role_name]["Average_Number"] = role_count # We rewrite the mean so that the next step cannot mess with the known information
        
    # Produce the proper total number of players by adjusting the outliers back to the mean
    residues = {}
    for role_index in range(num_roles):
        role_stats = role_stats_list[role_index]
        if role_counts[role_index] > role_stats["Min_Number"]: # Make sure within valid range to check
            residues.update({role_index:role_counts[role_index] - role_stats["Average_Number"] + random.random()*epsilon}) #Tie-breaking
    ...
    for role_pair in sorted_residues[:np.abs(excess)]:
        if excess > 0:
            ...
            role_counts[role_pair[0]] += 1
    role_counts = np.array(role_counts)
